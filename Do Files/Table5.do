...
** TABLE 5
		...
		** Table 5A
			est clear
			eststo clear
			local women "women_work_prop women_decision CA women_mobility daughter_edu_high downstream2"	
			...
			esttab `women' using "$output/Table5A.csv", b(%10.2f) keep(status) se ///
		...
		** Table 5B
			...
			esttab `women' using "$output/Table5B.csv", b(%10.2f) keep(1.status 1.tar_group 1.status*1.tar_group) se ///
