** Relief from Usury: Impact of a Self-Help Group Lending Program in Rural India
		...
		** Table 2A
			...
			esttab `direct_vars' using "$output/Table2A.csv", b(%10.2f) keep(status) se ///
		...
		** Table 2B
			...
			esttab `direct_vars' using "$output/Table2B.csv", b(%10.2f) keep(1.status 1.tar_group 1.status*1.tar_group) se ///
